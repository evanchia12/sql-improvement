...
1. We can improve the performance of the query by adding/implementing index on each column that we use to search or filter in each related table.

2. Recommend use `=` instead of `LIKE`. For instance, change `JobCategories.name LIKE '%キャビンアテンダント%'` to `JobCategories.name = 'キャビンアテンダント'` since querying the exact value is faster than using `LIKE`.

3. Avoid doing join for each rows.
